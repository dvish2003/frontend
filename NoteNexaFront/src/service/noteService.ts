...
export const addNote = async (note: Note) => {
    const response = await apiClient_1.post("/notes/addNote", note);
    console.log("return response data",response.data);
    return response.data;
}
export const deleteNote = async (note: NoteDeleteForm) => {
    const response = await apiClient_1.post("/notes/deleteNote", note);
    console.log("return response data",response.data);
    return response.data;
}

export const getAllNote = async () => {
    const response = await apiClient_1.post("/notes/getAll");
    return response.data;
}